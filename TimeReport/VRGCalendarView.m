...
- (void)drawRect:(CGRect)rect
{
   ...
   CGRect rectangle = CGRectMake(0,0,self.frame.size.width,kVRGCalendarViewTopBarHeight);
    ...
    // menue color
   CGContextSetFillColorWithColor(context,[UIColor colorWithRed:0.522 green:0.596 blue:0.69 alpha:1].CGColor);// [UIColor colorWithRed:0.83 green:0.43 blue:0.57 alpha:0.5].CGColor); //[UIColor lightGrayColor].CGColor);//whiteColor
...
/*
    ...
    CGColorRef whiteColor = [UIColor colorWithHue:0.58 saturation:0.63 brightness:0.79 alpha:1].CGColor; 
    CGColorRef lightGrayColor = [UIColor colorWithRed:0.522 green:0.596 blue:0.69 alpha:1].CGColor;
    
    CGRect paperRect = self.bounds;
    
    drawLinearGradient(context, rectangle, lightGrayColor,whiteColor);    
    

    drawLinearGradient(context, paperRect, whiteColor, lightGrayColor);    CGContextSaveGState(context);
    addRoundedRectToPath(context, self.frame, 2, 2);
    CGContextClip(context);
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (selectedDate && i==selectedDateBlock) {
            ...
            CGContextSetFillColorWithColor(context, [UIColor colorWithRed:0.522 green:0.596 blue:0.69 alpha:1].CGColor);
            CGContextFillPath(context);
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        ...
        CGRect rectangle = CGRectMake(targetX+10,targetY -10,16,16);
        // Image
        UIImage *yourImage = [UIImage  imageNamed:@"note2.png"];
        [yourImage drawInRect: rectangle];
        //
...
-(id)init {
    ...
    if (self) {
        ...
        labelCurrentMonth.backgroundColor=[UIColor clearColor];//whiteColor 
